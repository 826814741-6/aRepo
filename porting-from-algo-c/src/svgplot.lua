...
local function gSvgPlot(writer, methodMaker)
	return function (width, height)
		assertInitialValue(width, height)

		local T = writer(width, height)

		T.pathStart = methodMaker("pathStart", pathStart)
		T.pathEnd = methodMaker("pathEnd", pathEnd)

		local move, draw = gMove(height), gDraw(height)

		T.move = methodMaker("move", move)
		T.moveRel = methodMaker("moveRel", moveRel)
		T.draw = methodMaker("draw", draw)
		T.drawRel = methodMaker("drawRel", drawRel)
		T.circle = methodMaker("circle", circle)
		T.ellipse = methodMaker("ellipse", ellipse)
		T.line = methodMaker("line", line)
		T.rect = methodMaker("rect", rect)
...
local svgPlot, svgPlotWholeBuffer, svgPlotWithBuffer =
	gSvgPlot(writer, makeMethod),
	gSvgPlot(writerWholeBuffer, makeMethodForWholeBuffer),
	gSvgPlot(writerWithBuffer, makeMethodForWithBuffer)
